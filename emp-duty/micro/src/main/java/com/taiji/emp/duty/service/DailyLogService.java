package com.taiji.emp.duty.service;

import com.taiji.emp.duty.entity.dailylog.DailyLog;
import com.taiji.emp.duty.repository.DailyLogRepository;
import com.taiji.emp.duty.searchVo.DailyLogSearchVo;
import com.taiji.emp.duty.vo.dailylog.DailyLogVo;
import com.taiji.micro.common.enums.DelFlagEnum;
import com.taiji.micro.common.service.BaseService;
import lombok.AllArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;
import org.springframework.stereotype.Service;
import org.springframework.util.Assert;

import java.time.LocalDateTime;
import java.util.List;

@Slf4j
@Service
@AllArgsConstructor
public class DailyLogService extends BaseService<DailyLog,String> {

    @Autowired
    private DailyLogRepository repository;

    /**
     * 新增值班日志信息
     * @param entity
     * @return
     */
    public DailyLog create(DailyLog entity) {
        Assert.notNull(entity,"entity对象不能为空");
        entity.setDelFlag(DelFlagEnum.NORMAL.getCode());
        DailyLog result = repository.save(entity);
        return result;
    }

    /**
     * 修改值班日志信息
     * @param entity
     * @return
     */
    public DailyLog update(DailyLog entity) {
        Assert.notNull(entity,"entity对象不能为空");
        entity.setDelFlag(DelFlagEnum.NORMAL.getCode());
        DailyLog result = repository.save(entity);
        return result;
    }

    /**
     * 删除某条值班日志信息
     * @param id
     * @param delFlagEnum
     */
    public void deleteLogic(String id, DelFlagEnum delFlagEnum) {
        Assert.hasText(id,"id不能为null或空字符串");
        DailyLog entity = repository.findOne(id);
        repository.deleteLogic(entity,delFlagEnum);
    }

    /**
     * 根据id查询某条值班日志信息
     * @param id
     * @return
     */
    public DailyLog findOne(String id) {
        Assert.hasText(id,"id不能为null或空字符串");
        DailyLog result = repository.findOne(id);
        return result;
    }

    /**
     * 根据条件查询值班日志列表——分页
     * @param searchVo
     * @param pageable
     * @return
     */
    public Page<DailyLog> findPage(DailyLogSearchVo searchVo, Pageable pageable) {
        Page<DailyLog> result = repository.findPage(searchVo,pageable);
        return result;
    }

    /**
     * 根据条件查询值班日志列表
     * @param searchVo
     * @return
     */
    public List<DailyLog> findList(DailyLogSearchVo searchVo) {
        List<DailyLog> list = repository.findList(searchVo);
        return list;
    }
}
