package com.taiji.emp.event.msgService;
...
import com.taiji.emp.event.msgService.feign.MsgNoticeClient;
import com.taiji.emp.event.msgService.feign.UserClient;
...
public class InfoMsgService {
     ...
     * @param eventVo
     */
    public void sendInfoNewMsg(EventVo eventVo,UserProfileVo profileVo) {
        CompletableFuture<Void> future = CompletableFuture.runAsync(() -> {
            if(null!=eventVo){
                List<Receiver> receivers = new ArrayList<>();
                String title = MsgSendGlobal.MSG_INFO_NEW_TITLE_SEND;
                String sendOrg = profileVo.getOrgId();
                String sendOrgName = profileVo.getOrgName();
                StringBuilder builder = new StringBuilder();
                builder.append("系统生成了新事件，事件名称为")
                        .append(eventVo.getEventName());
                String msgContent = builder.toString();
                MsgNoticeVo msgNoticeVo = new MsgNoticeVo();
                msgNoticeVo.setTitle(title);
                msgNoticeVo.setSendOrg(sendOrg);
                ...
                msgNoticeVo.setIcon(msgConfig.getIcon());
                msgNoticeVo.setPath(msgConfig.getPath());
                msgNoticeVo.setEntityId(eventVo.getId());

                ResponseEntity<UserVo> userVoResponseEntity = userClient.findByName(FeedbackGlobal.INFO_USER_NAME);
                UserVo userVo = ResponseEntityUtils.achieveResponseEntityBody(userVoResponseEntity);
                Receiver receiver = new Receiver();
                receiver.setId(userVo.getId());
                receiver.setName(FeedbackGlobal.INFO_USER_NAME);
                receivers.add(receiver);
                msgNoticeVo.setReceivers(receivers);
    ...
    public UserVo getUserVoInfo(){
        String userName = FeedbackGlobal.INFO_USER_NAME;
